...
// From https://docs.aws.amazon.com/amazonglacier/latest/dev/checksum-calculations.html
        ...
        */
        public static void Main(string[] args)
        {
            if (args.Length < 1)
            {
                ...
                Console.WriteLine("Usage: DN-GlacierCalcSha256 <filename|dirname> [pattern] [fn_start] [fn_end]");
            ...
            if (File.Exists(path))
            {
                Console.WriteLine($"File mode: '{path}'");
            ...
            {
                Console.WriteLine($"Directory Mode: '{path}'");
                ...
                string fn_start = null, fn_end = null;
                ...
                foreach (string fn in files)
                {
                    ...
                    if (fn_start != null && fn_end != null)
                    {
                        ok = false;
                        if (string.Compare(fn_start, fn2, true) <= 0 &&
                            string.Compare(fn_end, fn2, true) >= 0)
                        {
                            ok = true;
                        }
                    }
                    ...
                    {
                        Console.Write($"{fn2}: ");
                        Console.WriteLine(ComputeSHA256TreeHashStr(fn));
                    }
                }
            }
        }

        public static string ComputeSHA256TreeHashStr(string filename)
        {
            byte[] treeHash = ComputeSHA256TreeHash(filename);

            return BitConverter.ToString(treeHash).Replace("-", "").ToLower();
        }

        public static byte[] ComputeSHA256TreeHash(string filename)
        {
            using (FileStream inputFile = File.Open(filename, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
            {
                return ComputeSHA256TreeHash(inputFile);
            }
        }
        ...
        public static byte[][] GetChunkSHA256Hashes(FileStream file)
        {
            ...
            if (numChunks == 0)
            {
                return new byte[][] { CalculateSHA256Hash(new byte[0], 0) };
            }
